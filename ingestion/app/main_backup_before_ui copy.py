...
def get_blob_bytes(bucket_name: str, file_path: str) -> bytes:
    ...
    if not blob.exists():
        raise HTTPException(status_code=404, detail=f"File not found: gs://{bucket_name}/{file_path}")
...
def log_pipeline(run_id: str, batch_id: str, step_name: str, status: str, error_message: str = None) -> None:
...
def log_quality(run_id: str, batch_id: str, table_name: str, check_name: str, check_status: str,
                expected_value: str = None, actual_value: str = None, failed_count: int = 0) -> None:
...
def log_file_upload(run_id: str, batch_id: str, source_system: str, table_name: str,
                    source_file_name: str, gcs_uri: str, file_hash: str,
                    row_count: int, upload_status: str, error_message: str = None) -> None:
...
def log_rejected(run_id: str, batch_id: str, source_file_name: str, gcs_uri: str,
                 rejection_reason: str, payload: dict) -> None:
...
def copy_to_raw(bucket_name: str, tmp_file_path: str, source_system: str, table_name: str,
                batch_id: str, source_file_name: str) -> str:
...
def copy_to_rejected(bucket_name: str, tmp_file_path: str, source_system: str, table_name: str,
                     batch_id: str, source_file_name: str) -> str:
    load_date = utc_now().date().isoformat()
...
def load_to_bronze(df: pd.DataFrame, table_name: str, metadata: dict) -> int:
    ...
    # Bronze layer mein raw preservation ke liye columns ko STRING rakh rahe hain.
...
@app.get("/health")
def health_check():
    return {"status": "ok", "service": "ecom-ingestion-service"}
...
def process_file(request: ProcessRequest):
    ...
    try:
        ...
        if check_duplicate_file(file_hash):
            ...
            log_pipeline(run_id, request.batch_id, "duplicate_check", "SKIPPED", "Duplicate file detected")
